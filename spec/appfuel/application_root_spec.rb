  ...
  RSpec.describe ApplicationRoot do
    context '#handle_default_app_name' do
      it 'assigns the default app name when default_app is true' do
        ...
        app_container = build_container(root: 'some root module')

        init.handle_app_name(params, app_container, framework_container)
        expect(framework_container[:default_app_name]).to eq(params[:app_name])
      end
      ...
      it 'returns the app_name passed into params' do
        ...
        params = {app_name: 'bar'}
        framework_container = build_container
        app_container = build_container(root: mock_root)
